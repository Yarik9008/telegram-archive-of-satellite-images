...
class FastBot:
    ...
    def __init__(self) -> None:
        ...
        self.logs = Logger('arch_bot_all', "/home/lorett/telegram-archive-of-satellite-images/.logs", loggingLevels['debug'])
        ...
        with open("/home/lorett/telegram-archive-of-satellite-images/config_bot.json", 'r') as file_config:
            ...
            self.chat = self.config['chat_all']
        ...
        api_id = 25004944
        api_hash = '3d29770555fbca4b0ea880003ed892bc'
    ...
    def append_pass_list(self, data_path: Path):
        with open(str(os.path.dirname(os.path.abspath(__file__))) + "/pass_list_all.txt", 'a+') as file_pass:
            file_pass.write(str(data_path) + '\n')
            self.logs.info(f'append to pass list: {str(data_path)}')
    
    # чтение pass list 
    def read_pass_list(self):
        return [Path(i[:-1]) for i in open(str(os.path.dirname(os.path.abspath(__file__))) + "/pass_list_all.txt", 'r').readlines()]
    ...
    def send(self, data_path: Path):
        ...
        try: 
            ...
            file_old = str(data_path) + self.sat_config[sat_name]['data_file']
            file_new = str(data_path) + f'/{pass_sat}.cadu'
            os.rename(file_old, file_new)
            self.logs.debug(f'file rename: {file_new}')
            
            self.logs.info(f'start upload data file: {file_new}')
            self.client.send_file(self.chat, file_new)
            self.logs.info(f'finish upload data file: {str(file_new)}')
    ...
    def main(self):
        while True:
            ...
            for directory in new_data:
                ...
                while self.calculate_delay(directory) < self.difference + 10:
